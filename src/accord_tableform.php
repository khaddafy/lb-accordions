...
class KintassaAccordionTableForm extends KintassaOptionsTableForm {
	...
	function do_row_action_del($row_id) {
		// TODO: cascade-delete panels in the accordion
		if ($this->pager->delete($row_id)) {
			echo ("Accordion #{$row_id} deleted.");
		}
		return false;
	}
...
class KintassaAccordionRowOptionsForm extends KintassaRowForm {
	const Sort = 1;
	const Edit = 2;
	const Delete = 4;
	const All = 7;
	...
	function __construct($table_form, $row, $opts) {
		...
		if ($opts & KintassaAccordionRowOptionsForm::Sort) {
		...
		if ($opts & KintassaAccordionRowOptionsForm::Edit) {
			$edit_args = array("mode" => "accordion_edit", "id" => $row->id);
		...
		if ($opts & KintassaAccordionRowOptionsForm::Delete) {
...
class KintassaAccordionRowOptionsFactory extends KintassaRowFormFactory {
	function __construct($opts) {
		$this->opts = $opts;
	}

	function instanciate($table_form, $row) {
		return new KintassaAccordionRowOptionsForm($table_form, $row, $this->opts);
	}
}

class KintassaAccordionDBResultsPager extends KintassaPager {
	...
	function page_link($page_num) {
		$page_args = array("mode" => "accordion_list", "pagenum" => $page_num);
