...
abstract class KintassaAccordionForm extends KintassaForm {
	...
	function add_widgets($def) {
		...
		$accordion_applets = KintassaAccordionApplet::available_applets();
		foreach ($accordion_applets as $applet_name) {
			$applet_info = KintassaAccordionApplet::applet_info($applet_name);
	...
	function data() {
		...
		$display_mode_ok = in_array($dat['display_mode'], $accordion_applets);
		if (!$display_mode_ok) {
